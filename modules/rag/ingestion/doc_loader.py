...
def _read_doc(path: Path) -> str:
    try:
        ...
        logger.warning(f"mammoth failed for {path.name}: {e}")
        return ""
...
def ingest_doc(doc_path: str | Path) -> int:
    ...
    logger.info(f"Ingesting {doc_path.name} (spec: {spec_id})")
    ...
    objects = []
    for i, chunk in enumerate(tqdm(chunks, desc=f"Embedding {doc_path.name}", unit="chunk")):
        objects.append({
            "text": chunk, "source": doc_path.name, "doc_type": "docx",
            "spec_id": spec_id, "page_number": 0, "chunk_index": i,
    ...
    logger.success(f"Inserted {inserted} chunks from {doc_path.name}")
    return inserted
...
def ingest_vocabulary_docx(docx_path: str | Path) -> int:
    ...
    with get_client() as client:
        collection = client.collections.get(COLLECTION_NAME)
        with collection.batch.dynamic() as batch:
            for i, chunk in enumerate(tqdm(chunks, desc="Embedding vocabulary", unit="chunk")):
                batch.add_object(
                    properties={
                        "text": chunk, "source": doc_path.name, "doc_type": "vocabulary",
                        "spec_id": "3GPP_vocabulary", "page_number": 0,
                        "chunk_index": i, "section": _extract_section_heading(chunk),
                    },
                    vector=embed(chunk),
                )
                inserted += 1

    logger.success(f"Inserted {inserted} vocabulary chunks")
    return inserted
